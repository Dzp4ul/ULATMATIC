  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const parsed = JSON.parse(raw) as { id?: unknown; pio_name?: string; pio_email?: string };
        ...
        setPioId(id);
        if (parsed.pio_name) {
          setPioName(parsed.pio_name);
          setProfileName(parsed.pio_name);
        }
        if (parsed.pio_email) {
        ...
        const data = (await res.json()) as {
          ok?: boolean;
          user?: { pio_name?: string; pio_email?: string; profile_photo?: string | null };
        };

        if (!active || !res.ok || !data.ok || !data.user) return;
        if (data.user.pio_name) {
          setPioName(data.user.pio_name);
          setProfileName(data.user.pio_name);
        }
        if (data.user.pio_email) setProfileEmail(data.user.pio_email);
        ...
        >
          <div className="flex items-center gap-3 px-6 py-5">
            ...
            <div>
              ...
              <div className="text-xs text-white/80">PIO Portal</div>
            </div>
          </div>
          ...
          <div className="px-6 pb-4">
            <div className="flex items-center gap-3 rounded-xl bg-white/10 p-3">
              {profilePhotoUrl ? (
                <img src={profilePhotoUrl} alt="PIO" className="h-10 w-10 rounded-full object-cover" />
              ) : (
              ...
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold">{pioName || 'Public Information Officer'}</div>
                <div className="truncate text-xs text-white/80">Public Information Officer</div>
              </div>
            </div>
          </div>
              ...
              <div className="relative" ref={profileMenuRef}>
                ...
                >
                  <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-white/20">
                    {profilePhotoUrl ? (
                      <img src={profilePhotoUrl} alt="PIO" className="h-full w-full object-cover" />
                    ) : (
                      <User className="h-4 w-4 text-white/80" />
                    )}
                  </div>
                  <div className="hidden sm:block">
                    <div className="text-sm font-semibold leading-tight">{pioName || 'PIO'}</div>
                    <div className="text-xs text-white/70">Public Information Officer</div>
                  </div>
                ...
                {profileMenuOpen ? (
                  <div className="absolute right-0 mt-3 w-64 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
                    <div className="flex items-center gap-3 px-4 py-4">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-gray-900">{pioName || 'Public Information Officer'}</div>
                        <div className="text-xs text-gray-500">{profileEmail || 'PIO Account'}</div>
                        <div className="text-xs text-gray-400">Public Information Officer</div>
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                ...
                <form
                  ...
                  onSubmit={async (e) => {
                    e.preventDefault();
                    if (!pioId) {
                      setProfileError('PIO session not found. Please sign in again.');
                      return;
                    }
                    ...
                    try {
                      ...
                      fd.append('id', String(pioId));
                      fd.append('pio_name', profileName.trim());
                      ...
                      const nextName = user.pio_name ?? profileName.trim();
                      setPioName(nextName);
                      setProfileName(nextName);
                      setProfileEmail(user.pio_email ?? profileEmail.trim());
