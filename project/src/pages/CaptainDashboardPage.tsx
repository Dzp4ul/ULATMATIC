...
export default function CaptainDashboardPage({
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'profile' | 'residents' | 'complaints' | 'complaint_detail'>('dashboard');
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const raw = localStorage.getItem('ulatmatic_captain');
        if (!raw) {
          onNavigate('/signin');
          return;
        }

        const parsed = JSON.parse(raw) as { id?: unknown; cap_name?: string; cap_email?: string };
        ...
        setCaptainId(id);
        if (parsed.cap_name) {
          setCaptainName(parsed.cap_name);
          setProfileName(parsed.cap_name);
        }
        if (parsed.cap_email) {
        ...
        const data = (await res.json()) as {
          ok?: boolean;
          user?: { cap_name?: string; cap_email?: string; profile_photo?: string | null };
        };

        if (!active || !res.ok || !data.ok || !data.user) return;
        if (data.user.cap_name) {
          setCaptainName(data.user.cap_name);
          setProfileName(data.user.cap_name);
        }
        if (data.user.cap_email) setProfileEmail(data.user.cap_email);
        ...
        >
          <div className="flex items-center gap-3 px-6 py-5">
            ...
            <div>
              ...
              <div className="text-xs text-white/80">Captain Portal</div>
            </div>
          </div>
          ...
          <div className="px-6 pb-4">
            <div className="flex items-center gap-3 rounded-xl bg-white/10 p-3">
              {profilePhotoUrl ? (
                <img src={profilePhotoUrl} alt="Captain" className="h-10 w-10 rounded-full object-cover" />
              ) : (
              ...
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold">{captainName || 'Barangay Captain'}</div>
                <div className="truncate text-xs text-white/80">Barangay Captain</div>
              </div>
            </div>
          </div>
              ...
              <div className="relative" ref={profileMenuRef}>
                ...
                >
                  <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-white/20">
                    {profilePhotoUrl ? (
                      <img src={profilePhotoUrl} alt="Captain" className="h-full w-full object-cover" />
                    ) : (
                      <User className="h-4 w-4 text-white/80" />
                    )}
                  </div>
                  <div className="hidden sm:block">
                    <div className="text-sm font-semibold leading-tight">{captainName || 'Captain'}</div>
                    <div className="text-xs text-white/70">Barangay Captain</div>
                  </div>
                ...
                {profileMenuOpen ? (
                  <div className="absolute right-0 mt-3 w-64 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
                    <div className="flex items-center gap-3 px-4 py-4">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-gray-900">{captainName || 'Barangay Captain'}</div>
                        <div className="text-xs text-gray-500">{profileEmail || 'Captain Account'}</div>
                        <div className="text-xs text-gray-400">Barangay Captain</div>
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                ...
                <form
                  ...
                  onSubmit={async (e) => {
                    e.preventDefault();
                    if (!captainId) {
                      setProfileError('Captain session not found. Please sign in again.');
                      return;
                    }
                    ...
                    try {
                      ...
                      fd.append('id', String(captainId));
                      fd.append('captain_name', profileName.trim());
                      ...
                      const nextName = user.cap_name ?? profileName.trim();
                      setCaptainName(nextName);
                      setProfileName(nextName);
                      setProfileEmail(user.cap_email ?? profileEmail.trim());
