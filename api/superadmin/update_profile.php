...
$id = (int)($body['id'] ?? 0);
$name = trim((string)($body['superadmin_name'] ?? ''));
$email = trim((string)($body['superadmin_email'] ?? ''));
...
$stmt = $conn->prepare('SELECT superadmin_name, superadmin_email, profile_photo FROM superadmin_user WHERE id = ? LIMIT 1');
if (!$stmt) {
    $conn->close();
...
$name = $name !== '' ? $name : (string)($existing['superadmin_name'] ?? '');
$email = $email !== '' ? $email : (string)($existing['superadmin_email'] ?? '');
...
if (isset($_FILES['profile_photo']) && is_array($_FILES['profile_photo'])) {
    ...
    if ($err !== UPLOAD_ERR_NO_FILE) {
        if ($err !== UPLOAD_ERR_OK) {
            $conn->close();
            api_send_json(400, ['ok' => false, 'error' => 'Failed to upload profile photo']);
        }
...
$stmt = $conn->prepare('UPDATE superadmin_user SET superadmin_name = ?, superadmin_email = ?, profile_photo = ? WHERE id = ?');
if (!$stmt) {
    $conn->close();
