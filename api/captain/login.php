...
$stmt = $conn->prepare('SELECT id, captain_name AS cap_name, captain_email AS cap_email, captain_pass AS cap_pass FROM captain_user WHERE captain_email = ? LIMIT 1');
if (!$stmt) {
    $conn->close();
    api_send_json(500, [
        'ok' => false,
...
$statusCheck = $conn->query("SHOW COLUMNS FROM captain_user LIKE 'status'");
if ($statusCheck && $statusCheck->num_rows > 0 && $user) {
    $stmtS = $conn->prepare('SELECT status FROM captain_user WHERE id = ? LIMIT 1');
    if ($stmtS) {
...
$storedPass = (string)($user['cap_pass'] ?? '');
$ok = false;
