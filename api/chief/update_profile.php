...
$id = (int)($body['id'] ?? 0);
$chiefName = trim((string)($body['chief_name'] ?? ''));
$chiefEmail = trim((string)($body['chief_email'] ?? ''));
...
$stmt = $conn->prepare('SELECT chief_name, chief_email, profile_photo FROM chief_user WHERE id = ? LIMIT 1');
if (!$stmt) {
    $conn->close();
    api_send_json(500, [
        'ok' => false,
...
$chiefName = $chiefName !== '' ? $chiefName : (string)($existing['chief_name'] ?? '');
$chiefEmail = $chiefEmail !== '' ? $chiefEmail : (string)($existing['chief_email'] ?? '');
...
if (isset($_FILES['profile_photo']) && is_array($_FILES['profile_photo'])) {
    ...
    if ($err !== UPLOAD_ERR_NO_FILE) {
        ...
        $uploadsRelDir = 'uploads/profiles/chief';
        $uploadsAbsDir = $uploadsRoot . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'profiles' . DIRECTORY_SEPARATOR . 'chief';
...
$stmt = $conn->prepare('UPDATE chief_user SET chief_name = ?, chief_email = ?, profile_photo = ? WHERE id = ?');
if (!$stmt) {
    $conn->close();
    api_send_json(500, [
        'ok' => false,
...
$stmt->bind_param('sssi', $chiefName, $chiefEmail, $profilePath, $id);
$stmt->execute();
$stmt->close();
$conn->close();
