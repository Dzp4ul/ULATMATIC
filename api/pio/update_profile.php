...
$id = (int)($body['id'] ?? 0);
$pioName = trim((string)($body['pio_name'] ?? ''));
$pioEmail = trim((string)($body['pio_email'] ?? ''));
...
$stmt = $conn->prepare('SELECT pio_name, pio_email, profile_photo FROM pio_user WHERE id = ? LIMIT 1');
if (!$stmt) {
    $conn->close();
    api_send_json(500, [
        'ok' => false,
...
$pioName = $pioName !== '' ? $pioName : (string)($existing['pio_name'] ?? '');
$pioEmail = $pioEmail !== '' ? $pioEmail : (string)($existing['pio_email'] ?? '');
...
if (isset($_FILES['profile_photo']) && is_array($_FILES['profile_photo'])) {
    ...
    if ($err !== UPLOAD_ERR_NO_FILE) {
        ...
        $uploadsRelDir = 'uploads/profiles/pio';
        $uploadsAbsDir = $uploadsRoot . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'profiles' . DIRECTORY_SEPARATOR . 'pio';
...
$stmt = $conn->prepare('UPDATE pio_user SET pio_name = ?, pio_email = ?, profile_photo = ? WHERE id = ?');
if (!$stmt) {
    $conn->close();
    api_send_json(500, [
        'ok' => false,
...
$stmt->bind_param('sssi', $pioName, $pioEmail, $profilePath, $id);
$stmt->execute();
$stmt->close();
$conn->close();
